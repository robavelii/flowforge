...
- Milestone 8: Billing, Quotas & Platform Extras
- Plan/subscription/usage models with seeded Free and Pro plans
- Quota enforcement for executions (hard 429), storage uploads, and plan API rate
- Usage analytics (`GET /v1/quotas`, `GET /v1/billing/usage`) and Stripe-ready billing APIs
- Tenant settings and per-workspace feature flags with Redis-backed evaluation cache
- JSON Patch (`application/json-patch+json`) workflow updates and bulk archive/delete
- Admin outbox list/replay and workspace system metrics summary
- Node registry `supportsDryRun` markers; sandbox executions remain side-effect free and quota-exempt
- M8 e2e coverage for quotas, flags, JSON Patch, bulk archive, and admin metrics
